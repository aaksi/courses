...
const arraySortInfo = (inputArray) => {
  let arrSort = [...inputArray].sort((a, b) => a - b)
  let arrSortRevers = [...inputArray].sort((a, b) => b - a)

  if (inputArray.find(el => typeof el === 'string')) return "Некоторые элементы не являются числами"
  if (inputArray.slice().toString() != arrSort.toString() && inputArray.slice().toString() != arrSortRevers) return 'Массив не отсортирован'
  if (inputArray.slice().toString() === arrSortRevers.toString()) return "Массив отсортирован по убыванию"
  if (inputArray.slice().toString() === arrSort.toString()) return "Массив отсортирован по возрастанию"
}
